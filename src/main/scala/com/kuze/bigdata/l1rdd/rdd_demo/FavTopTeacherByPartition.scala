...
object FavTopTeacherByPartition {

  def main(args: Array[String]): Unit = {

    val conf = new SparkConf().setAppName("FavTopTeacherByPartition").setMaster("local[4]")
    val sc = new SparkContext(conf)

    //指定以后从哪里读取数据
    val lines: RDD[String] = sc.textFile("/Users/baiye/Downloads/spark-03-TopN与WordCount执行过程详解/课件与代码/teacher.cclog")
    //整理数据
    ...
    val sorted: RDD[((String, String), Int)] = reduced.mapPartitions(it => {
      ...
      //即排序，有不全部加载到内存
      //长度为5的一个可以排序的集合
      /*val empty: List[((String, String), Int)] = List()
      for (elem <- it) {
        empty.
        elem
      }*/
    ...
    sorted.saveAsTextFile("/Users/baiye/Downloads/spark-03-TopN与WordCount执行过程详解/课件与代码/result")
